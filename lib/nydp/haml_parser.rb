...
module Nydp
  class HamlParser
    ...
    def compute_tabs(line)
      ...
      message = ::Haml::Error.message(:inconsistent_indentation,
    ...
    def script(line, escape_html = nil, preserve = false)
      raise ::Haml::SyntaxError.new(::Haml::Error.message(:no_ruby_code, '=')) if line.text.empty?
    ...
    def tag(line)
      ...
      when '='
        parse = true
        if value[0] == ?=
          value = ::Haml::Util.unescape_interpolation(value[1..-1].strip)
      ...
      when '&', '!'
        if value[0] == ?= || value[0] == ?~
          ...
          if value[1] == ?=
            value = ::Haml::Util.unescape_interpolation(value[2..-1].strip)
            escape_interpolation = true if escape_html
            escape_html = false
          else
            value = value[1..-1].strip
          end
        elsif ::Haml::Util.contains_interpolation?(value)
          value = ::Haml::Util.unescape_interpolation(value)
          escape_interpolation = true if escape_html
          parse = true
          escape_html = false
        end
      else
        if ::Haml::Util.contains_interpolation?(value)
          value = ::Haml::Util.unescape_interpolation(value)
          escape_interpolation = true if escape_html
          parse = true
          escape_html = false
      ...
      attributes = ::Haml::Parser.parse_class_and_id(attributes)
      ...
      raise ::Haml::SyntaxError.new(::Haml::Error.message(:illegal_nesting_self_closing), @next_line.index) if block_opened? && self_closing
      raise ::Haml::SyntaxError.new(::Haml::Error.message(:no_ruby_code, action), last_line - 1) if parse && value.empty?
      raise ::Haml::SyntaxError.new(::Haml::Error.message(:self_closing_content), last_line - 1) if self_closing && !value.empty?

      if block_opened? && !value.empty? && !is_ruby_multiline?(value)
        raise ::Haml::SyntaxError.new(::Haml::Error.message(:illegal_nesting_line, tag_name), @next_line.index)
      end
    ...
    def comment(text)
      ...
      if block_opened? && !text.empty?
        raise ::Haml::SyntaxError.new(::Haml::Error.message(:illegal_nesting_content), @next_line.index)
      end
    ...
    def doctype(text)
      raise ::Haml::SyntaxError.new(::Haml::Error.message(:illegal_nesting_header), @next_line.index) if block_opened?
      version, type, encoding = text[3..-1].strip.downcase.scan(DOCTYPE_REGEX)[0]
      ParseNode.new(:doctype, @line.index + 1, :version => version, :type => type, :encoding => encoding)
    end

    def filter(name)
      raise Error.new(::Haml::Error.message(:invalid_filter_name, name)) unless /^\w+$/.match?(name)
    ...
    def parse_new_attributes(text)
      ...
      loop do
        ...
        if name == false
          ...
          raise ::Haml::SyntaxError.new(::Haml::Error.message(:invalid_attribute_list, text.inspect), last_line - 1)
        end
      ...
      attributes.each do |name, (type, val)|
        if type == :static
          ...
          # dynamic_attributes << "#{::Haml::Util.inspect_obj(name)} => #{val},"
        end
      end
      dynamic_attributes << "}"
      dynamic_attributes = nil if dynamic_attributes == "{}"

      return [static_attributes, nil], scanner.rest, last_line
    end
    ...
    def parse_new_attribute(scanner)
      ...
      loop do
        ...
        content << [:static, balance(scanner, ?{, ?}, 1).first[0...-1]]
      end

      return name, [:static, content.first[1]] if content.size == 1
      return name, [:static,
        %!"#{content.each_with_object(+'') {|(t, v), s| s << (t == :str ? ::Haml::Util.inspect_obj(v)[1...-1] : "\#{#{v}}")}}"!]
    end
